...
게시글 상세 페이지의 Next.js/React 렌더링 흐름과 오류 처리를 개선합니다.

- 렌더링 구조를 조정하더라도 게시글 상세 페이지의 기존 UI, SEO 메타데이터, API 요청 계약은 유지한다.
- 클라이언트 컴포넌트는 상호작용이 필요한 범위로 한정하고, 서버에서 가능한 데이터 조회와 렌더링을 우선한다.
- 변경 후 오류 경계가 의도한 범위에서 동작하는지 확인하며, 관련 없는 페이지나 공용 컴포넌트는 수정하지 않는다.
